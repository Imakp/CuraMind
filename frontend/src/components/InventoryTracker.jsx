...
const InventoryTracker = ({ 
  ...
  useEffect(() => {
    setFormData({
      ...
      sheetCount: sheetSize > 0 ? Math.floor(totalTablets / sheetSize).toString() : '0',
  ...
  const handleChange = (field, value) => {
    ...
    if (field === 'sheetCount' || field === 'sheetSize') {
      const sheets = field === 'sheetCount' ? parseInt(value) || 0 : parseInt(formData.sheetCount) || 0
      const size = field === 'sheetSize' ? parseInt(value) || 0 : parseInt(formData.sheetSize) || 0
      
      if (sheets >= 0 && size > 0) {
        const calculatedTablets = sheets * size
        setFormData(prev => ({ ...prev, totalTablets: calculatedTablets.toString() }))
      }
    }

    // Validate on change for immediate feedback
    if (inputMode === 'sheets' && (field === 'sheetCount' || field === 'sheetSize')) {
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-4">
      ...
      <div className="space-y-4">
        {inputMode === 'tablets' ? (
          /* Tablets Input Mode */
          <div>
            <label htmlFor="totalTablets" className="block text-sm font-medium text-gray-700 mb-1">
              Total Tablets
            </label>
            <input
              type="number"
              ...
              className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50 disabled:text-gray-500 ${
                errors.totalTablets ? 'border-red-300' : 'border-gray-300'
              }`}
              placeholder="Enter total number of tablets"
            />
            {errors.totalTablets && (
              <p className="mt-1 text-sm text-red-600">{errors.totalTablets}</p>
            )}
            
            {showConversion && parseInt(formData.sheetSize) > 0 && (
              <p className="mt-2 text-sm text-gray-600">
                Equivalent: {currentEquivalent.sheets} sheet(s)
                {currentEquivalent.remainder > 0 && ` + ${currentEquivalent.remainder} tablet(s)`}
              </p>
            )}
          </div>
        ) : (
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label htmlFor="sheetCount" className="block text-sm font-medium text-gray-700 mb-1">
                  Number of Sheets
                </label>
                <input
                  type="number"
                ...
                />
                {errors.sheetCount && (
                  <p className="mt-1 text-sm text-red-600">{errors.sheetCount}</p>
                )}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  disabled={disabled}
                  className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50 disabled:text-gray-500 ${
                    errors.sheetSize ? 'border-red-300' : 'border-gray-300'
                  }`}
                  placeholder="10"
                />
                {errors.sheetSize && (
                  <p className="mt-1 text-sm text-red-600">{errors.sheetSize}</p>
                )}
              </div>
            </div>
            ...
            <div className="p-3 bg-gray-50 rounded-md">
              <p className="text-sm text-gray-600">
                <span className="font-medium">Total tablets:</span> {formData.totalTablets || 0}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="border-t border-gray-200 pt-4">
          <h4 className="text-sm font-medium text-gray-900 mb-2">Current Inventory</h4>
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium text-gray-900">
                ...
                {totalTablets % sheetSize > 0 && ` + ${totalTablets % sheetSize} tablets`}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={() => {
              ...
              const newTotal = currentTotal + (parseInt(formData.sheetSize) || sheetSize || 10)
              setFormData(prev => ({ 
                ...prev, 
                totalTablets: newTotal.toString(),
                sheetCount: Math.floor(newTotal / (parseInt(formData.sheetSize) || sheetSize)).toString()
              }))
            }}
            disabled={disabled}
          ...
          <button
            ...
            onClick={() => {
              const currentTotal = parseFloat(formData.totalTablets) || 0
              const newTotal = Math.max(0, currentTotal - (parseInt(formData.sheetSize) || sheetSize || 10))
              setFormData(prev => ({ 
                ...prev, 
                totalTablets: newTotal.toString(),
                sheetCount: Math.floor(newTotal / (parseInt(formData.sheetSize) || sheetSize)).toString()
